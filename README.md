...
ASFAR 1.1.5 uses native WordPress pages, menus, posts and featured images, with named ACF Pro fields and Polylang translations.
...
Build with `python3 tools/package.py v1.1.5` (PHP and Node.js required). The release workflow packages tagged versions for native WordPress updates from GitHub.
...
Version 1.1.5 matches the September 11 source update, including homepage copy, persistent banner buttons, destination controls, PIF artwork, portfolio interaction and corrected news spacing. The one-time upgrade sets Arabic as the default and preserves translated menu destinations.
