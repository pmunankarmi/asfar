...
?>
	<footer class="mt-dk-footer" id="contact">
		...
		<div class="mt-dk-wrap">
			<div class="mt-dk-footer__in">
				<?php echo wp_get_attachment_image( asfar_option( 'footer_logo' ), 'full', false, array( 'class' => 'mt-dk-footer__logo', 'alt' => asfar_option( 'site_name' ) ) ); ?>
				<a class="mt-dk-footer__item" href="<?php echo esc_url( 'mailto:' . asfar_option( 'contact_email' ) ); ?>">
			...
			<div class="mt-asfar-social">
				<?php asfar_menu( 'footer' ); ?>
				<?php while ( function_exists( 'have_rows' ) && have_rows( 'social_links', 'asfar_shared' ) ) : the_row(); ?>
					<a href="<?php echo esc_url( get_sub_field( 'url' ) ); ?>"><?php echo esc_html( asfar_translate( get_sub_field( 'label' ) ) ); ?></a>
				<?php endwhile; ?>
			</div>
		</div>
	</footer>
