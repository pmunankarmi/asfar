...
(async()=>{
 ...
 for(const key of ['index','index-ar']){
  ...
  assert.equal(await page.locator("#mt-dkMap").getAttribute('data-hot'),'yanbu');
  ...
  await page.locator("#mt-about").screenshot({path:'/private/tmp/asfar-latest-about-'+key+'.png'});
 ...
 assert.ok((await page.locator("#mt-heroSlider").getAttribute('class')).includes("mt-is-film"));
 assert.equal(await page.locator("#mt-heroMorphName").textContent(),'The Coast');
 await page.screenshot({path:'/private/tmp/asfar-latest-film.png'});
 await page.evaluate(()=>{const m=document.querySelector("#mt-dkMap");window.__lenis?.scrollTo(m.offsetTop-50,{immediate:true,force:true});if(!window.__lenis)window.scrollTo(0,m.offsetTop-50);});await page.waitForTimeout(1600);
