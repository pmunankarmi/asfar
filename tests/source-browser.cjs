...
(async()=>{
 ...
 for(const key of ['index','index-ar']){
  ...
  assert.equal(await page.locator(".mt-hero__slide").count(),3);assert.equal(await page.locator(".mt-hero__dot").count(),2);
  await page.locator(".mt-hero__dot").nth(1).click(); await page.waitForTimeout(50);
 ...
 await page.goto(urls.index,{waitUntil:'networkidle'});await page.locator(".mt-hero__dot").nth(1).click();await page.waitForTimeout(2400);
