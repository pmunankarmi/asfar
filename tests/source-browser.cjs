...
(async()=>{
 ...
 for(const key of ['index','index-ar']){
  ...
  assert.equal(await page.locator('.hero__slide').count(),3);assert.equal(await page.locator('.hero__dot').count(),4);
  await page.locator('.hero__dot').nth(2).click(); await page.waitForTimeout(50);
  assert.equal(await page.locator('.hero__dot').nth(2).getAttribute('aria-pressed'),'true');
  assert.equal(await page.locator('.hero__cta--fixed button').isVisible(),true);
  assert.equal(await page.locator('.dk-clickable').count(),3);
  await page.locator('.dk-clickable[data-region="yanbu"]').focus();await page.keyboard.press('Enter'); await page.waitForTimeout(50);
  assert.equal(await page.locator('#dkMap').getAttribute('data-hot'),'yanbu');
  assert.equal(await page.locator('.asfar-portfolio-pane:not([hidden])').count(),1);
  const mask=await page.locator('.dk-about__pif').evaluate(e=>getComputedStyle(e).maskImage);assert.ok(mask.includes('/uploads/'));
 ...
 await page.goto(urls.index,{waitUntil:'networkidle'});await page.locator('.hero__dot').nth(3).click();await page.waitForTimeout(2400);
 assert.ok((await page.locator('#heroSlider').getAttribute('class')).includes('is-film'));
 ...
 const pinned=await page.locator('html').evaluate(e=>e.classList.contains('dk-maplock'));
 console.log('Map pinned',pinned); assert.equal(pinned,true); if(pinned){await page.keyboard.press('Escape');assert.equal(await page.locator('html').evaluate(e=>e.classList.contains('dk-maplock')),false);}
